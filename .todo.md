...
[] Add available only in constructor `on` function for subscribe to action, call, signal. (Alternative syntax of subscribe to actions)
[] Add available only in constructor `effect` function for subscribe to any side effect.
...
[] Next slide about unit combination/composition
...
```javascript
const unit = useUnit(Unit, [...args: any[]]); // Nooo... Think about It.
```
