...
import { set_create, set_add, set_has, set_delete, set_size, set_extract, set_free } from "./wasm";
...
function box_deep_invalidate(next_bound) {
  ...
  do {
    ...
    for (const x of bound) {
      ...
      if (rels) {
        for (const r of set_extract(rels)) {
          box_invalid.add(r);
          next_bound.add(r);
        }
      }
...
function tick_finish() {
  ...
  do {
    ...
    for (const [id, expr] of new Map(box_expr)) {
      ...
      if (!deps) continue;
      for (const d of set_extract(deps)) {
...
function slice_deps_close() {
  ...
  for (const d of set_extract(slice_deps)) {
    let rels = box_rels.get(d);
    if (!rels) {
      box_rels.set(d, (rels = set_create()));
    }
...
function box(value) {
  ...
  return {
    get() {
      if (slice_deps) set_add(slice_deps, id);
      return value;
    },
    set(val) {
...
function print_graph(next_tick) {
  ...
  for (const [x, deps] of box_deps) {
    line.push(x, box_deps.size, set_extract(deps));
  }
  line.push("<>");
  for (const [x, rels] of box_rels) {
    line.push(x, box_rels.size, set_extract(rels));
  }
