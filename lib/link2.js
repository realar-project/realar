...
function unlink_map_exclude(scope_id, map, id) {
  let excludes = unlink_map_exludes.get(scope_id);
  if (!excludes) {
    unlink_map_exludes.set(scope_id, (excludes = []));
  }
  excludes.push([map, id]);
}

function unlink_array_exclude(scope_id, arr, val) {
  let excludes = unlink_array_exludes.get(scope_id);
  if (!excludes) {
    unlink_array_exludes.set(scope_id, (excludes = []));
  }
  excludes.push([arr, val]);
}

function unlink_fn(scope_id, fn) {
  let list = unlink_notify.get(scope_id);
  if (!list) {
    unlink_notify.set(scope_id, (list = []));
  }
  list.push(fn);
}

function unlink_box_collection_free(scope_id, collection_id) {
  let list = unlink_box_collections.get(scope_id);
  if (!list) {
    unlink_box_collections.set(scope_id, (list = []));
  }
  list.push(collection_id);
}

function unlink_box_free(scope_id, box_id) {
  let list = unlink_boxs.get(scope_id);
  if (!list) {
    unlink_boxs.set(scope_id, (list = []));
  }
  list.push(box_id);
}
