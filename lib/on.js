...
const
  text_on_action_incorrect = `Only action, signal and call supported as first argument for "on" function`,
  text_on_func_incorrect = `Only function supported as second argument for "on" function`;
...
function on(action, fn) {
  if (!action) {
    throw new Error(text_on_action_incorrect);
  }
  if (!fn || !(fn instanceof Function)) {
    throw new Error(text_on_func_incorrect);
  }
