import { is_event_uniq, resolve_event_uniq, event_listen } from "./event";
...
const
  ...
  unit_key = Symbol("unit"),
  unit_not_initialized_key = Symbol("unit_not_initialized"),
  unit_get_meta = Symbol("unit_get_meta"),
  unit_inst_key = Symbol("unit_inst");
...
function unit(schema) {
  const factory = function factory() {
    ...
    Object.keys(schema_descriptors).forEach(key => {
      ...
      if (is_event_uniq(key)) {
        const id = resolve_event_uniq(key);
        events.set(id, value);
        return;
      }
    ...
    factory[unit_key] = [meta_methods];
    factory[unit_not_initialized_key] = false; // TODO: remove it
  ...
  // TODO: remove all down it

  factory[unit_not_initialized_key] = true;

  factory[unit_get_meta] = function() {
    let inst = {};
    let meta_methods = []
    link_scope(inst);
    ...
    Object.keys(schema_descriptors).forEach(key => {
      if (keyword_keys.has(key)) return;
      const { value } = schema_descriptors[key];

      if (is_event_uniq(key)) return;
      else if (is_call_uniq(key)) return;
      else if (is_signal_uniq(key)) return;

      if (typeof value === "function" && !is_connector(value)) {
        meta_methods.push(key);
      }
    });
