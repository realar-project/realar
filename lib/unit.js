import {
  box_value_create, box_value_get_phase, box_value_set_phase, box_collection_start, box_collection_finish,
  box_expr_create, box_expr_start, box_expr_finish, box_computed_create, box_computed_start, box_computed_finish, box_entry_start, box_entry_finish
} from "./box";
import { link, link_scope_create, link_scope_start, link_scope_finish, unlink_fn, unlink, unlink_box_collection_free, get_current_link_scope_id } from "./link";
...
function box_create(val, link_scope_id) {
  ...
  return {
    get: () => (box_value_get_phase(box_id), val),
    set: (v) => {
      if (Object.is(val, v)) return;
      if (link_id) link_id = unlink(link_id), 0;
      val = v;
      if (v && v[unit_inst_key]) link_id = link(v, link_scope_id);
      box_value_set_phase(box_id);
    }
  };
}

function unsupported_manual_call_of(method) {
  return function () {
    throw new Error(`Manual call of unit "${method}" unsupported`);
  }
}

let unit_inst_proto = {
  [unit_inst_key]: true,
  constructor: unsupported_manual_call_of("constructor"),
  destructor: unsupported_manual_call_of("destructor"),
  expression: unsupported_manual_call_of("expression")
};
...
function unit(_schema) {
  ...
  function factory() {
    ...
    -- schema_phase_deep;
    let collection_id = box_collection_finish();
