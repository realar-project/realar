import { core, fns, ready } from "./core";
import { is_connector, connector } from "./connector";
import { link, link_scope, link_scope_finish, unlink_fn, unlink } from "./link";
import { get_mock } from "./test";

ready(copy_core_api_fns, unit2, core);

const
  unit_meta_methods_key = Symbol(),
  unit_key = Symbol(),
  unit_inst_key = Symbol()
  ;

let
  schema_deep = 0;

export {
  is_schema_phase,
  unit_key,
  unit_meta_methods_key,
  unit_inst_key,
  unit2
}

function schema_phase_start() {
  ++schema_deep;
}

function schema_phase_finish() {
  --schema_deep;
}

function is_schema_phase() {
  return schema_deep;
}

function unit2(_schema) {
  let
    a = arguments,
    make = a[0],
    values = a[1],
    computeds = a[2],
    methods = a[3]
  ;

  function factory() {
    if (is_schema_phase()) {
      const schema_arguments = arguments;
      return connector(function() {
        const inst = factory.apply(null, schema_arguments);
        link(inst);
        return {
          get: () => inst
        }
      });
    }

    const mock = get_mock(factory);
    if (mock) return mock.apply(this, arguments);

    let ctx = {};

    link_scope(ctx);

    schema_phase_start();
    let raw = make.call(ctx);
    schema_phase_finish();

    let i = 3;
    for (let name of values) {
      let val = raw[i++];
      let id = core.b0(); /*box_value_create*/
      let linked = 0;
      Object.defineProperty(ctx, name, {
        get: () => (core.b1(id) /*box_value_get_phase*/, val),
        set: (v) => {
          if (Object.is(val, v)) return;
          if (linked) linked = unlink(val), 0;
          val = v;
          if (v && v[unit_inst_key]) linked = link(v), 1;
          core.b2(id); /*box_value_set_phase*/
        }
      });
    }

    for (let name of computeds) {
      Object.defineProperty(ctx, name, {
        get: raw[i++]
      });
    }
    for (let name of methods) {
      Object.defineProperty(ctx, name, {
        value: raw[i++]
      });
    }

    if (raw[0]) { /* constructor */
      raw[0].apply(ctx, arguments);
    }
    if (raw[1]) { /* destructor */
    }
    if (raw[2]) { /* expression */
      raw[2]();
    }

    return ctx;
  }
  factory[unit_meta_methods_key] = methods;
  return factory;
}

function copy_core_api_fns(u, core) {
  for (let i = 0; i <= 9; i++) {
    u["b"+i] = core["b"+i]
  }
  let from = "a".charCodeAt(0);
  let to = "e".charCodeAt(0);
  for (let i = from; i <= to; i++) {
    let n = String.fromCharCode(i);
    u["b"+n] = core["b"+n]
  }
  u.fns = fns;

/*#if DEBUG
  let dec_map = {
    b0: "box_value_create",
    b1: "box_value_get_phase",
    b2: "box_value_set_phase",
    b3: "box_expr_create",
    b4: "box_expr_start",
    b5: "box_expr_finish",
    b6: "box_computed_create",
    b7: "box_computed_start",
    b8: "box_computed_finish",
    b9: "box_entry_start",
    ba: "box_entry_finish",
    bb: "box_view_create",
    bc: "box_view_start",
    bd: "box_view_finish",
    be: "box_free"
  }
  for (const k of Object.keys(dec_map)) {
    let name = dec_map[k];
    u[k] = function() {
      const args = Array.prototype.slice.call(arguments);
      const res = core[k].apply(core, args);
      // console.log(name, ...args, res);
      return res;
    }
  }
//#endif */
}
