import { core, fns, box_expr_create, box_expr_start, box_expr_finish } from "./core";
import { link, link_scope_create, link_scope_start, link_scope_finish, unlink, unlink_map_exclude, unlink_box_free } from "./link";
...
export {
  ...
  view_render_start,
  view_render_finish,
  view_set_skip_first_deep,
  useUnit,
  useService
}

function view_set_skip_first_deep(f = 1) {
  view_skip_first_deep = f;
}
...
function view_render_start() {
  ...
  if (view_skip_first_deep) {
    if (view_deep !== 2) return;
  } else {
    if (view_deep !== 1) return;
  }
...
function useUnit(factory) {
  ...
  const args = Array.prototype.slice(arguments).slice(1);
